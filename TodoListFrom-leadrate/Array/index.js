...
]; //помещаем JSON файл в переменную, иначе, в данных условиях его редактировать невозможно
...
const strr = () => {
  ...
  for (let i = 0; i < todosJson.length; i++) {
    document.querySelector(
      ".main__todos__todo"
    ).innerHTML += `<div id='${i}' class='component'>
      <input type="checkbox" ${todosJson[i].completed ? "checked" : ""}/>
      <label calss='crossOut' for="checkbox_todo">
        <span calss='activeChecked'></span>
        <div class="title  ${todosJson[i].completed ? "" : "titleHover"}">${
      todosJson[i].title
    } <br />
            <div class="descr">
            ${
              todosJson[Math.round(1 - 0.5 + Math.random() * (199 - 1 + 1))] //описание для каждого задание выпадает рандомно
                .title
            }  </div
            ></div> 
      </label>
...
component.forEach((i) =>
  i.addEventListener("click", (e) => {
    e.currentTarget.childNodes[1].click(); // В зависимости от наличия атрибута "checked" у "input" меняем на противоположный
    todosJson[i.id].completed = i.childNodes[1].checked; // Меняем значение "completed" в массиве "todosJson"
  })
);
